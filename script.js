class Enemy {
  constructor(game) {
    ...
    this.speedX = 0;
    this.speedY = 0;
    this.speedModifier = (Math.random() * 0.5 + 0.1).toFixed(2);
    this.angle = 0;
    this.collided = false;
    this.free = true; //1
  }
...
class Projectile {
  ...
  update() {
    if (!this.free) {
      this.x += this.speedX;
      this.y += this.speedY;
      if (this.x < 0 || this.x > this.game.width || this.y < 0 || this.y > this.game.height) this.reset();
    }
  }
}
class Player {
...
class Game {
  constructor(canvas) {
    ...
    this.projectilePool = []; //5
    this.numberOfProjectiles = 25; //6
    this.createProjectilePool(); //7
...
this.drawStatusText(context);
  ...
  drawStatusText(context) {
    ...
    for (let i = 0; i < this.lives; i++) {
      context.fillRect(20 + 15 * i, 50, 10, 30);
    }
    ...
    let message2;
      ...
      message1 = "You win!";
       message2 = `Your score is ${this.score}. Play again press  R`;
    } else if (this.lives <= 0 ) {
       message1 = "You lose!";
       message2 = "Try again press R";
    }
     context.font = "100px Impact";
    context.fillText(message1, this.width * 0.5, this.height * 0.33);
    context.font = "40px Impact";
    context.fillText(message2, this.width * 0.5, this.height * 0.65);
    }
    context.restore();
  }
  calcAim(a, b) {
